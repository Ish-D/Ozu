...
namespace ops {
    inline float getFloat(const Tensor& tensor, size_t index) {
        ...
        if (tensor.dataType == DataType::bf16) return bf16Tof32(static_cast<const uint16_t*>(tensor.data)[index]);

        // No other data types implemented
        std::unreachable();
    }
    ...
    void matmul(Tensor& out, const Tensor& a, const Tensor& b) {
        ...
        int seqLen = (a.shape.size() > 1) ? a.shape[0] : 1;
        ...
        for (int s = 0; s < seqLen; s++) {
            for (int i = 0; i < outFeatures; i++) {
                ...
                for (int j = 0; j < inFeatures; j++) {
                    float aVal = getFloat(a, s * inFeatures + j);
                    float bVal = getFloat(b, i * inFeatures + j);

                    sum += aVal * bVal;
                }
                outData[s * outFeatures + i] = sum;
            }
        }
    }

    void rmsNorm(Tensor& out, const Tensor& a, const Tensor& weight, float eps) {
        const int dim  = a.shape.back();
        const int seqLen = a.numElements() / dim;

        auto* outData = static_cast<float*>(out.data);

        for (int s = 0; s < seqLen; s++) {
            const int rowOffset = s * dim;
            ...
            float invRMS = 1.0f / std::sqrt((sumSquares / dim) + eps);

            for (int i = 0; i < dim; i++) {
                float normalizedVal = getFloat(a, rowOffset + i) * invRMS;
                outData[rowOffset + i] = normalizedVal * getFloat(weight, i);
            }
        }
    }
    ...
    void applyRope(Tensor &q, Tensor &k, int position, float ropeTheta, int headDim) {
        ...
        const int    seqLen    = q.shape[0];
        const size_t qRowSize  = q.shape[1];
        const size_t kRowSize  = k.shape[1];

        for (int s = 0; s < seqLen; s++) {
            // Process Q (Split Halves)
            for (int h = 0; h < qRowSize; h += headDim) {
                for (int d = 0; d < headDim / 2; d++) {
                    int i1 = s * qRowSize + h + d;
                    int i2 = s * qRowSize + h + d + headDim / 2; // Pair with the element half-way across the head
            ...
            for (int h = 0; h < kRowSize; h += headDim) {
                for (int d = 0; d < headDim / 2; d++) {
                    int i1 = s * kRowSize + h + d;
                    int i2 = s * kRowSize + h + d + headDim / 2;
    ...
    void updateCache(Tensor& cache, const Tensor& val, int startPos) {
        const int rowSize = cache.shape[1];
        const int seqLen  = val.shape[0];
        assert(val.shape[1] == rowSize             && "[updateCache] Value size must match cache row size");
        assert(startPos + seqLen <= cache.shape[0] && "[updateCache] Position exceeds maximum cache sequence length");

        auto* cacheData = static_cast<float*>(cache.data);
        for (int s = 0; s < seqLen; s++) {
            for (int i = 0; i < rowSize; i++) {
                cacheData[(startPos + s) * rowSize + i] = getFloat(val, s * rowSize + i);
            }
        }
    }
    ...
    void attention(Tensor &out, const Tensor &q, const Tensor &kCache, const Tensor &vCache, Tensor& scoresScratch, int numQHeads, int numKVHeads, int headDim, int startPos) {
        const int kvMul     = numQHeads / numKVHeads;
        const float scale   = 1.0f / std::sqrt(static_cast<float>(headDim));
        const int seqLen    = q.shape[0];
        const int qRowSize  = numQHeads * headDim;
        const int kvRowSize = numKVHeads * headDim;

        auto* outData = static_cast<float*>(out.data);
        auto* kCacheData = static_cast<const float*>(kCache.data);
        auto* vCacheData = static_cast<const float*>(vCache.data);

        assert(scoresScratch.numElements() >= static_cast<size_t>(startPos + seqLen) && "[attention] scoresScratch too small.");
        auto* scores = static_cast<float*>(scoresScratch.data);

        for (int s = 0; s < seqLen; s++) {
            const int numTokens = startPos + s + 1;
            ...
            for (int h = 0; h < numQHeads; h++) {
                ...
                for (int t = 0; t < numTokens; t++) {
                    ...
                    for (int d = 0; d < headDim; d++) {
                        float qVal = getFloat(q,  s * qRowSize + h * headDim + d);
                        float kVal = kCacheData[t * kvRowSize + kvHead * headDim + d];
                ...
                for (int d = 0; d < headDim; d++) {
                    float outVal = 0.0f;
                    for (int t = 0; t < numTokens; t++) {
                        float vVal = vCacheData[t * kvRowSize + kvHead * headDim + d];
                        outVal += scores[t] * vVal;
                    }

                    outData[s * qRowSize + h * headDim + d] = outVal;
                }
            }
        }
    }
